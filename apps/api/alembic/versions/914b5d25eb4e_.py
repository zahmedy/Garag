...
def upgrade() -> None:
    ...
    op.add_column('carlisting', sa.Column('make', sqlmodel.sql.sqltypes.AutoString(), nullable=True))
    op.add_column('carlisting', sa.Column('model', sqlmodel.sql.sqltypes.AutoString(), nullable=True))

    # Backfill make/model from existing lookup tables before dropping them.
    op.execute(
        sa.text(
            """
            UPDATE carlisting AS cl
            SET make = cmk.name, model = cmd.name
            FROM carmodel AS cmd
            JOIN carmake AS cmk ON cmk.id = cmd.make_id
            WHERE cl.model_id = cmd.id
            """
        )
    )
    op.execute(sa.text("UPDATE carlisting SET make = COALESCE(make, 'unknown')"))
    op.execute(sa.text("UPDATE carlisting SET model = COALESCE(model, 'unknown')"))

    op.alter_column('carlisting', 'make', nullable=False)
    op.alter_column('carlisting', 'model', nullable=False)
...
def downgrade() -> None:
    ...
    op.add_column('carlisting', sa.Column('model_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.add_column('carlisting', sa.Column('make_id', sa.INTEGER(), autoincrement=False, nullable=True))
